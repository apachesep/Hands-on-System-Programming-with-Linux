/*
 * assessments/ch12:sigtmwtinfo.c
 * 
 ***************************************************************
 ...
 ****************************************************************
 * Assignment:
 * Enhance the sigwt program to use the sigtimedwait(2) API. Set the timeout
 * at 1.5 seconds. If no signals is received within this time, print a simple
 * "timed out" message to the console and continue waiting. Further, trap
 * fatal kernel signals (like SIGSEGV and SIGFPE) via the sigaction(2) API
 * (and not via the sigtimedwait(2)). Arrange to have the kernel generate
 * SIGSEGV (by deliberately inserting a bug in your code), and handle the
 * resulting segfault, displaying diagnostics (as in assignment #1 above).
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
