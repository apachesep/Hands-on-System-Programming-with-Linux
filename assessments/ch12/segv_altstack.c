 ...
 *
 * From:
 *  Ch 12 : Signalling - Part II : Assignments
 ****************************************************************
 * Qs: As seen in the previous chapter, the altstack.c program can suffer
 * a segmentation fault when it's alternate signal stack is overflowed
 * (by too many signals). Write a SIGSEGV handler in the sa_sigaction
 * prototype style to properly handle the case. It should display the
 * signal origin (si_code) value and meaning as well as the faulting
 * instruction or memory reference (address) that caused the segfault
 * to occur.
 *
